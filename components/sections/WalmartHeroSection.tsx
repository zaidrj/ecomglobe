/**
 * Walmart Hero Section Component
 * 
 * Hero section for Walmart Development page matching Figma design exactly.
 * Includes "Walmart Marketplace Management" heading, subtitle, two CTA buttons, and decorative Walmart logo elements.
 * 
 * @component
 * @example
 * ```tsx
 ...
 * - Decorative Walmart logo elements positioned around hero
 * - Background decorative elements (ellipses, rectangles)
 * 
 * @returns {JSX.Element} Walmart Development hero section with heading, subtitle, and CTAs
 */
...
import type { WalmartHeroSectionProps } from '@/types';

export default function WalmartHeroSection({
  title = "Walmart Marketplace Management",
  subtitle = "Launch, optimize, and scale your Walmart business with our comprehensive suite of Walmart-specific services. From account setup to catalog management, we handle everything.",
  primaryCtaText = "Get Started Today",
  primaryCtaHref = "/contact",
  secondaryCtaText = "View Success Stories",
  secondaryCtaHref = "/case-studies",
  className = "",
}: WalmartHeroSectionProps) {
  return (
    <div className={`relative min-h-[500px] md:min-h-[600px] flex items-center justify-center overflow-visible px-4 md:px-0 w-full ${className}`}>
      ...
      {/* Walmart logo decorative elements - Hidden on mobile */}
      <div className="hidden lg:flex absolute h-[427.418px] items-center justify-center left-[-129px] top-[683px] w-[427.05px] rotate-[34.046deg]">
        <OptimizedImage
          src="/images/walmart/hero/walmart-hero.png"
          alt=""
          width={308}
          height={308}
